...
* **Modeless Execution**: The interface floats on top of Revit, utilizing an `IExternalEventHandler` ("Universal Handler") to route database transactions back to Revit's main thread.
* **Live Filtering & Rich Text Previews**: The list filters itself as you type into the "Find" field. Changes in the list are previewed in bold text via WPF `Run` objects before you commit them.
* **Match Case Toggle**: Includes a checkbox to force case-sensitive searches, powered by Python's regex (`re`) module.
* **Contextual Navigation**: Right-click any row in the DataGrid to select that object or navigate to it (`ShowElements`) within your active Revit view. 
...
3. Type your search string into the **Find** field. The DataGrid will filter the list of elements as you type.
4. Type your desired replacement string into the **Replace** field. The "New Name" column will preview the changes with bold text formatting.
...
6. *(Optional)* Right-click any row in the table to **Select** the element in the model or **Show** it in the active view. Note that attempting to "Show" non-physical elements (like Materials) will alert you that they do not exist in graphical views.
